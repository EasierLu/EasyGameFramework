...
namespace EGFramework.Runtime.Fsm
{
    /// <summary>
    /// 有限状态机。
    /// </summary>
    ...
    public sealed class Fsm<T> : FsmBase, IReference where T : class
    {
        ...
        public static Fsm<T> Create(string name, T owner, params FsmState<T>[] states)
        {
            if (owner == null)
            {
                throw new FsmException("FSM owner is invalid.");
            }

            if (states == null || states.Length < 1)
            {
                throw new FsmException("FSM states is invalid.");
            }
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                {
                    throw new FsmException(Util.Utility.Text.Format("FSM '{0}' state '{1}' is already exist.", new TypeNamePair(typeof(T), name), stateType.FullName));
                }
        ...
        public static Fsm<T> Create(string name, T owner, List<FsmState<T>> states)
        {
            ...
            foreach (FsmState<T> state in states)
            {
                ...
                if (fsm.m_States.ContainsKey(stateType))
                {
                    throw new FsmException(Utility.Text.Format("FSM '{0}' state '{1}' is already exist.", new TypeNamePair(typeof(T), name), stateType.FullName));
                }
        ...
        public void Start<TState>() where TState : FsmState<T>
        {
            if (IsRunning)
            {
                throw new FsmException("FSM is running, can not start again.");
            }

            FsmState<T> state = GetState<TState>();
            if (state == null)
            {
                throw new FsmException(Utility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), typeof(TState).FullName));
            }
        ...
        public void Start(Type stateType)
        {
            ...
            if (stateType == null)
            {
                throw new FsmException("State type is invalid.");
            }

            if (!typeof(FsmState<T>).IsAssignableFrom(stateType))
            {
                throw new FsmException(Utility.Text.Format("State type '{0}' is invalid.", stateType.FullName));
            }

            FsmState<T> state = GetState(stateType);
            if (state == null)
            {
                throw new FsmException(Utility.Text.Format("FSM '{0}' can not start state '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), stateType.FullName));
            }
        ...
        public void GetAllStates(List<FsmState<T>> results)
        {
            if (results == null)
            {
                throw new FsmException("Results is invalid.");
            }
        ...
        public bool HasData(string name)
        {
            ...
            {
                throw new FsmException("Data name is invalid.");
            }
        ...
        internal void ChangeState(Type stateType)
        {
            if (m_CurrentState == null)
            {
                throw new FsmException("Current state is invalid.");
            }

            FsmState<T> state = GetState(stateType);
            if (state == null)
            {
                throw new FsmException(Utility.Text.Format("FSM '{0}' can not change state to '{1}' which is not exist.", new TypeNamePair(typeof(T), Name), stateType.FullName));
            }
